...
import {user_sign_off} from "@/logic/account/login";
...
export default function Example({children}) {
    ...
    const navigation = [
        { name: 'Classes', href: '/dashboard/classes', icon: UsersIcon, current: pathname === '/dashboard/classes' },
        { name: 'My reservation', href: '/dashboard/reservation', icon: CalendarIcon, current: pathname === '/dashboard/reservation'},
    ]

    const signing_off = async () => {
        setLoading(true)
        const response = await user_sign_off();
        if (response === true) {
            router.push('/login')
        } else {
            console.log("error")
